...
function Category(props) {
 ...
 toast.warn('Please select a category image');
 return;
 }
 ...
 if (res.ok) {
 toast.success(actionType === 'ADD' ? 'Category added successfully' : 'Category updated successfully');
 closeModal();
 ...
 <h2 className="title-admin mb-0" style={{ fontSize: '24px', fontWeight: '600', color: '#c5a059', marginLeft: '0', paddingLeft: '0' }}>Category Management
 ...
 <InputGroup>
 <InputGroup.Text className="bg-dark text-white border-end-0">
 ...
 {searchTerm && (
 <InputGroup.Text
 className="bg-dark text-white border-start-0 cursor-pointer"
 ...
 <button className="btn btn-success ms-3 d-flex align-items-center gap-2" onClick={openAddModal}> + Add New</button>
 </div>
 </div>
 ...
 <tbody>
 {isSearching ? (
 <tr><td colSpan="4" className="text-center py-4">Searching...</td></tr>
 ...
 <label className="fw-medium mb-1">Category Name <span className="text-danger">*</span></label>
 <input required type="text" className="form-control" name="name" value={formData.name} onChange={handleInputChange} placeholder="Enter category name..." />
 </div>

 <div className="form-group mb-4">
 <label className="fw-medium mb-1">Feature Image {actionType === 'ADD' && <span className="text-danger">*</span>}</label>
 ...
 <span className="text-muted fs-6">Current Image</span>
 </div>
 )}
 </div>

 <div className="modal-actions pt-3 border-top mt-auto">
 <button type="button" className="btn btn-secondary px-4 me-3 rounded-3" onClick={closeModal} disabled={actionLoading}>Cancel</button>
