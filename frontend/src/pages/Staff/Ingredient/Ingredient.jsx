...
function Ingredient() {
 ...
 console.error(error);
 toast.error('Error khi tải danh sách nguyên liệu');
 } finally {
 setLoading(false);
 ...
 const result = window.confirm('Are you sure you want to delete? nguyên liệu này?');

 if (result && id) {
 try {
 const response = await fetch(`/api/ingredient/${id}`, { method: 'DELETE' });
 if (response.ok) {
 toast.success('Delete nguyên liệu thành công');
 ...
 toast.error('Error khi xóa nguyên liệu');
 }
 } catch (error) {
 ...
 if (res.ok) {
 toast.success(actionType === 'ADD' ? 'Add nguyên liệu thành công' : 'Update nguyên liệu thành công');
 closeActionModal();
 ...
 <h2 className="title-admin mb-0" style={{ fontSize: '24px', fontWeight: '600', color: '#2d3748', marginLeft: '0', paddingLeft: '0' }}>Management Ingredients
 ...
 <Form.Control
 type="text"
 placeholder="Tìm theo tên, order vị, ghi chú..."
 value={searchTerm}
 ...
 title={sortOrder === 'desc' ?"Sort: Lớn -> Bé" :"Sort: Bé -> Lớn"}
 >
 {sortOrder === 'asc' ? <FaSortAmountUp className="text-success" /> : <FaSortAmountDownAlt className="text-success" />}
 Quantity ({sortOrder === 'desc' ?"Lớn → Bé" :"Bé → Lớn"})
 </button>
 <button className="btn btn-success ms-3 d-flex align-items-center gap-2" onClick={openAddModal} style={{ padding: '10px 22px', borderRadius: '8px', fontWeight: '500' }}> + Add nguyên liệu</button>
 </div>
 </div>
 ...
 <td><span className="badge rounded-pill bg-secondary px-3 py-2 border">{item.qty}</span></td>
 <td>{item.unit}</td>
 <td>{item.note || '-'}</td>
 <td>
 <span className={`ingredient-status ${item.is_active ? 'active' : 'inactive'}`}>
 {item.is_active ? 'Active' : 'Đã khóa'}
 </span>
 </td>
 <td>
 ...
 {actionType === 'ADD' ? 'Add nguyên liệu mới' : 'Update nguyên liệu'}
 </h4>

 <form onSubmit={handleActionSubmit} className="modal-body-scroll modal-form d-flex flex-column">
 <div className="modal-form-grid">
 <div className="form-group full-width">
 <label className="fw-medium mb-1">Name nguyên liệu <span className="text-danger">*</span></label>
 <input required type="text" className="form-control" name="name" value={formData.name} onChange={handleInputChange} placeholder="Enter tên nguyên liệu..." />
 </div>
 <div className="form-group">
 <label className="fw-medium mb-1">Quantity <span className="text-danger">*</span></label>
 ...
 <div className="form-group">
 <label className="fw-medium mb-1">Unit VNDo <span className="text-danger">*</span></label>
 ...
 <div className="form-group full-width">
 <label className="fw-medium mb-1">Note chi tiết</label>
 <textarea className="form-control" name="note" rows="2" value={formData.note} onChange={handleInputChange} placeholder="Enter ghi chú thêm (nếu có)..."></textarea>
 </div>
 <div className="form-group full-width d-flex flex-column">
 <label className="fw-medium mb-1">Status hiện tại</label>
 ...
 <button type="button" className="btn btn-secondary px-4 me-3 rounded-3" onClick={closeActionModal} disabled={actionLoading}>Cancel thao tác</button>
 ...
 {actionType === 'ADD' ? 'Confirm tạo mới' : 'Confirm thay VNDổi'}
 </button>
 </div>
 </form>
 </div>
