...
function Notification() {
    ...
    useEffect(() => {
        ...
        return () => {
            // Đánh dấu tất cả là VNDã VNDọc KHI RỜI KHỎI TRANG (cleanup)
    ...
    const handleShare = (item) => {
        const content = `[THÔNG BÁO] ${item.message} ${item.tableNumber ? `(Table ${item.tableNumber})` : ""}`;
    ...
    const sortedNotifications = [...filteredNotifications].sort((a, b) => {
        ...
        // Chỉ VNDẩy thông báo loại 'order' VNDã phục vụ xuống cuối
        ...
        // Đơn mới nhất (createdAt mới nhất) nằm ở first
            ...
            <Container>
                <div className="notification-header d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center">
                        ...
                        <h2 className="mb-0">System Notification</h2>
                    </div>
                    <div className="notification-filters d-flex gap-2">
                        ...
                        <button className={`btn-filter ${filter === 'order' ? 'active' : ''}`} onClick={() => setFilter('order')}>Orders</button>
                        <button className={`btn-filter ${filter === 'payment' ? 'active' : ''}`} onClick={() => setFilter('payment')}>Payment</button>
                        <button className={`btn-filter ${filter === 'support' ? 'active' : ''}`} onClick={() => setFilter('support')}>Request support</button>
                    </div>
                </div>
                ...
                <div className="notification-list mt-4">
                    {sortedNotifications.length > 0 ? (
                                                    ...
                                                    item.status === 'SERVED' ? 'Đã phục vụ' :
                                                        item.status === 'COMPLETED' ? 'Đã thanh toán' :
                                                            item.status === 'SERVING' ? 'Đang phục vụ' :
                                                                item.status === 'CONFIRMED' ? 'Đã nhận order' : 'Đang xử lý'}
                                            </span>
                                        )}
                                    </div>
                                    <span className="time">
                        ...
                        <div className="text-center py-5 text-muted bg-dark text-white rounded shadow-sm">
                            <FaBell className="fs-1 mb-3 opacity-25" />
                            <p className="mb-0">No support requests yet</p>
                        </div>
                    )}
                </div>
            </Container>
