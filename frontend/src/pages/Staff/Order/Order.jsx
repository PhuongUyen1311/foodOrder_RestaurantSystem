    ...
    return (
        <div className="staff-management block-order ps-0 pt-0">
            <div className="staff-management__header d-flex justify-content-between align-items-center mb-4 px-0">
                <h2 className="title-admin mb-0" style={{ fontSize: '24px', fontWeight: '600', color: '#c5a059', marginLeft: '0', paddingLeft: '0' }}>Order Management
 ...
 />
                    ...
                    <div className="search-container" style={{ width: '280px' }}>
                        <InputGroup>
                            ...
                            <Form.Control
                                type="text"
                                placeholder="Order ID, Customer, Qty..."
                                value={searchTerm}
                    ...
                    <button
                        ...
                        title={sortOrder === 'desc' ? "Sort: Total tiền Lớn -> Bé" : "Sort: Total tiền Bé -> Lớn"}
                    >
                        {sortOrder === 'asc' ? <FaSortAmountUp className="text-success" /> : <FaSortAmountDownAlt className="text-success" />}
                        Total Amount ({sortOrder === 'desc' ? "High → Low" : "Low → High"})
                    </button>
                </div>
            </div>
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td>
                                            <span className={order_source === 'online' ? 'admin-badge admin-badge--info' : 'admin-badge admin-badge--warning'}>
                                                {order_source === 'online' ? 'Online Order' : `Table ${table_number}`}
                                            </span>
                                        </td>
                                        <td>{total_item}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div className="d-flex flex-column gap-1 align-items-center">
                                                <span className={`admin-badge ${status === statusOrder.NEW ? 'admin-badge--default' :
                                                    ...
                                                    {
                                                        status === statusOrder.NEW ? 'New' :
                                                            status === statusOrder.CONFIRMED ? 'Confirmed' :
                                                                status === statusOrder.PROCESSING ? 'Processing' :
                                                                    status === statusOrder.COMPLETED ? 'Completed' : 'Cancelled'
                                                    }
                                                </span>
                                                <span className={`badge ${is_payment ? 'bg-success' : 'bg-secondary'}`} style={{ fontSize: '11px', padding: '4px 8px' }}>
                                                    {is_payment ? 'Paid' : 'Unpaid'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="text-center">
                                            <div className="d-flex justify-content-center gap-2">
                                                <Link to={`/staff/order/detail/${id}`} className="btn btn-sm btn-link p-1" title="Order Details">
