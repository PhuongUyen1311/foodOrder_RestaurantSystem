...
function OrderDetail() {
    ...
    const handleUpdateItemStatus = async (itemId, newStatus) => {
        ...
        if (res && res.success) {
            ...
            toast.error(res?.message || 'Error cập nhật trạng thái món');
        }
    };

    const handleUndoSplit = async () => {
        if (!window.confirm("Bạn có chắc chắn muốn hủy chia bill và gộp lại thành 1 hóa order tổng không?")) return;
        try {
            ...
            if (res && res.success) {
                ...
                toast.error(res?.message || 'Error hủy chia hóa order');
            }
        } catch (error) {
            toast.error('Error hệ thống');
        }
    };

    const slipItems = (orderDetail && orderDetail.is_payment)
        ? []
        : orderItems.filter(item => !item.status || (item.status !== 'SERVED' && item.status !== 'CANCELED'));
    const receiptItems = (orderDetail && orderDetail.is_payment)
        ? orderItems.filter(item => item.status !== 'CANCELED')
    ...
    slipItems.forEach(item => {
        const key = `Batch ${item.batch_num || 1} - ${item.guest_name || orderDetail?.guest_name || 'Guest chung'}`;
    ...
    useEffect(() => {
        ...
        socket.on('paymentSuccess', (data) => {
            ...
            // Kiểm tra xem order thành công có phải là order VNDang xem hoặc split bill của order này không
            ...
            if (isMainOrder || isSplitOrder) {
                toast.success(`Orders #${orderDetail.id} has been thanh toán thành công!`);
                setMainQrData(null);
                setQrModalData(null);
                setIsZoomed(false);
    ...
    const handlePayment = async (orderId, method = 'cash') => {
        if (orderId) {
        ...
        if (method === 'transfer') {
            try {
                ...
                if (data.success && data.paymentUrl) {
                    ...
                    alert(data.message || "Error tạo mã QR");
                }
            } catch (e) {
                console.error(e);
            }
    ...
    const handleMainPaymentQR = async () => {
        try {
            ...
            console.error(e);
            alert("Error kết nối khi tạo QR");
        }
    };
    ...
    const processSplitPayment = async (splitId, method) => {
        try {
            ...
            if (data.success) {
                ...
                if (window.confirm("Payment Successful. Bạn có muốn in hóa order cho khách này không?")) {
                    setTimeout(() => {
                        setPrintSplitBill(splitId);
                    }, 300);
                }
            } else {
                alert(data.message || "Error thanh toán phần chia bill");
            }
        } catch (e) {
            console.error(e);
        }
            ...
            setTimeout(() => {
                ...
                if (element) {
                    ...
                    html2pdf().set(opt).from(element).outputPdf('bloburl').then((pdfUrl) => {
                        ...
                        console.error('Error xuất PDF:', err);
                        toast.update(toastId, { render: "Error xuất PDF", type: "error", isLoading: false, autoClose: 3000 });
                    }).finally(() => {
    ...
    if (isPrinting) {
        ...
        return (
            <div className="bg-dark text-white print-section" style={{ color: '#000' }}>
                <style>{`
 @media print {
 ...
 `}</style>
                {billsToPrint.map((bill, bIdx) => {
                    ...
                    const statusLabel = (isSplitBill ? bill.is_payment : orderDetail.is_payment) ? "Đã Thanh Toán:" : "Guest Phải Thanh Toán:";

                    return (
                        <div key={bIdx} className="p-4 bg-dark text-white bill-container" style={{
                            borderBottom: bIdx < billsToPrint.length - 1 ? '3px dashed #ccc' : 'none',
                        }}>
                            <div className="text-center mb-4 border-bottom pb-3">
                                <h2 className="fw-bold text-uppercase mb-1" style={{ color: '#2c3e50' }}>VietNam Cuisine Restaurant</h2>
                                <p className="text-muted mb-0">Address: 123 Đường Ẩm Thực,Quận 1, TP. HCM</p>
                                <p className="text-muted mb-0">Hotline: 0909 123 456</p>
                                <h4 className="mt-3 fw-bold text-uppercase">{printTitle}</h4>
                                <p className="text-muted mb-0">Hóa order #: {orderDetail.id}</p>
                                <p className="text-muted mb-0 fw-bold mt-1">Người thanh toán: {payerName}</p>
                                {orderDetail.table_number && (
                                    <p className="text-muted mb-0">Table số: {orderDetail.table_number}</p>
                                )}
                            </div>
                            <Table variant="dark" striped className="text-end align-middle table-borderless mt-3 border-bottom pb-3">
                                        ...
                                        bill.split_type === 'item' ? (
                                            bill.items && bill.items.map((it, idx) => (
                                                <tr key={idx} className="bill-item-row">
                                                    <td className="text-start fw-bold">{it.product_name}</td>
                                                    <td className="text-center fw-bold">{it.qty}</td>
                                                    <td className="text-muted">{(it.unit_price || it.price)?.toLocaleString()} VND</td>
                                                    <td className="fw-bold">{(it.total_price || (it.price * it.qty) || 0).toLocaleString()} VND</td>
                                                </tr>
                                            ))
                                        ) : (
                                            orderItems && orderItems.map((it, idx) => (
                                                <tr key={idx} className="bill-item-row">
                                                    <td className="text-start fw-bold">{it.product_name || it.name}</td>
                                                    <td className="text-center fw-bold">{it.qty}</td>
                                                    <td className="text-muted">{(it.unit_price || it.price)?.toLocaleString()} VND</td>
                                                    <td className="fw-bold">{(it.total_price || (it.price * it.qty) || 0).toLocaleString()} VND</td>
                                                </tr>
                            ...
                            {isSplitBill && bill.split_type !== 'item' && (
                                <div className="d-flex justify-content-between w-100 mt-2 align-items-center mb-1">
                                    <span className="text-muted">Total bill gốc:</span>
                                    <span className="fw-bold fs-6">{(orderDetail?.total_price || 0).toLocaleString()} VND</span>
                                </div>
                            )}

                            {isSplitBill && bill.split_type !== 'item' && (
                                <div className="d-flex justify-content-between w-100 mb-2 border-bottom pb-2">
                                    <span className="text-muted fst-italic">Phần share bill ({bill.split_type === 'even' ? `Chia VNDều ${orderDetail.split_bills?.length || 0}` : `Tỷ lệ ${bill.percent}%`}):</span>
                                    <span className="fw-bold text-primary">{(bill.percent || (100 / (orderDetail.split_bills?.length || 1))).toFixed(1)}%</span>
                                </div>
                            )}

                            <div className="d-flex justify-content-between w-100 mt-3 align-items-center">
                                <span className="fw-bold fs-6">{statusLabel}</span>
                                <h4 className="text-danger fw-bold mb-0">
                                    {isSplitBill ? bill.amount?.toLocaleString() : orderDetail.total_price?.toLocaleString()} VND
                                </h4>
                            </div>
                ...
                <div className="position-fixed top-0 start-0 w-100 h-100 bg-dark bg-opacity-75 d-flex justify-content-center align-items-center" style={{ zIndex: 1050 }}>
                    <div className="vietqr-card">
                        ...
                        <div className="d-flex justify-content-center gap-2 mt-3">
                            <button className="btn btn-sm btn-outline-secondary" onClick={() => setQrModalData(null)}>Cancel</button>
                    ...
                    <div className={isZoomed ? "modal-qr-zoomed w-100" : ""}>
                        <div className="vietqr-card shadow-lg" style={{ minWidth: isZoomed ? '500px' : '400px' }}>
                            ...
                            <div className="d-flex justify-content-center gap-3 mt-3">
                                <button className="btn btn-outline-secondary" onClick={() => { setMainQrData(null); setIsZoomed(false); }}>Close</button>
                                <button className="btn btn-success fw-bold" onClick={() => {
                                    if (mainQrData.isSplit) {
                                        ...
                                        handlePayment(orderDetail.id, 'transfer');
                                    }
                                    setMainQrData(null);
                                    setIsZoomed(false);
                                }}>Đã nhận transfer</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className='order__detail'>
                <div className="d-flex justify-content-between align-items-center mb-4 no-print">
                    <h3 className="title-admin mb-0">
                        {orderDetail && orderDetail.is_payment ? "Details Hóa Đơn" : "Details Đơn Hàng"}: #{orderDetail && orderDetail.id}
                    </h3>
 ...
 Messenger
                        ...
                        {/* <button className="btn border border-secondary bg-dark text-white d-flex align-items-center shadow-sm" onClick={() => handleExportPDF('A4')}>
 Khổ A4
 </button> */}
                ...
                <div className="order__detail-container background-radius shadow-sm bg-dark text-white print-section">
                    <div className="text-center mb-4 border-bottom pb-3">
                        <h2 className="fw-bold text-uppercase mb-1" style={{ color: '#2c3e50' }}>VietNam Cuisine Restaurant</h2>
                        <p className="text-muted mb-0">Address: 123 Đường Ẩm Thực,Quận 1, TP. HCM</p>
                        <p className="text-muted mb-0">Hotline: 0909 123 456</p>
                        <h4 className="mt-3 fw-bold text-uppercase">
                            {orderDetail && orderDetail.is_payment ? "Hóa Đơn Thanh Toán" : "Phiếu mua hàng (Subtotal)"}
                        </h4>
                        <p className="text-muted mb-0">
                            {orderDetail && orderDetail.is_payment ? "Hóa order #" : "Phiếu mua hàng #"}: {orderDetail && orderDetail.id}
                        </p>
                    </div>

                    <div className="order__detail-head border p-3 rounded bg-secondary mb-4 d-flex justify-content-between align-items-center flex-wrap">
                        <div className="d-flex align-items-center gap-4">
                            <div>
                                <span className="text-muted me-1">Name khách hàng:</span>
                            ...
                            {orderDetail && orderDetail.table_number && (
                                <div className="text-primary fw-bold fs-5" style={{ borderLeft: '2px solid #ccc', paddingLeft: '20px' }}>
                                    Table số: {orderDetail.table_number}
                                </div>
                            )}
                        </div>
                        <div>
                            <span className="text-muted me-1">Date VNDặt hàng:</span>
                    ...
                    <div className="order__detail-group mt-3 d-flex flex-wrap" style={{ gap: '1rem' }}>
                        ...
                        {!(orderDetail && orderDetail.is_payment) && (
                            <div className="slip-section" style={{ flex: `0 0 calc(${leftColPercent}% - 0.5rem)`, maxWidth: `calc(${leftColPercent}% - 0.5rem)` }}>
                                <div className="border rounded bg-dark text-white shadow-sm h-100 no-print">
                                    <h5 className="p-3 bg-dark text-white rounded-top mb-0 fst-italic">Phiếu VNDặt món</h5>
                                    <div className="p-2">
                                        {Object.keys(slipBatches).length > 0 ? (
                                            Object.keys(slipBatches).map((batchKey, i) => (
                                                <div key={i} className="mb-3 border p-2 rounded bg-secondary">
                                                    <div className="d-flex justify-content-between align-items-center bg-secondary p-2 mb-2 border-bottom">
                                                    ...
                                                    {slipBatches[batchKey].map((item, index) => (
                                                        <div key={index} className="d-flex justify-content-between align-items-center bg-dark text-white p-2 mb-2 shadow-sm rounded border-start border-4 border-warning">
                                                            <div className="w-50">
                                                                <div className="fw-bold">{item.product_name} <span className="text-primary">(x{item.qty})</span></div>
                                                                <div className="text-muted" style={{ fontSize: '12px' }}>Status: <span className={item.status === 'PREPARING' ? 'text-warning fw-bold' : 'text-danger fw-bold'}>{item.status || 'NEW'}</span></div>
                                                            </div>
                                                            <div className="d-flex flex-column gap-1 w-50">
                                                                ...
                                                                <button className="btn btn-sm btn-outline-danger w-100 fw-bold mt-1" onClick={() => {
                                                                    if (window.confirm('Confirm hủy món này do sự cố quy trình? (Tiền sẽ VNDược trừ vào tổng order)')) {
                                            ...
                                            <div className="text-center p-4 text-muted fst-italic">Không có món nào VNDang chuẩn bị</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div style={{
                            ...
                            <div className="border rounded bg-dark text-white shadow-sm h-70 print-section">
                                <h5 className="p-3 bg-success text-white rounded-top mb-0 fst-italic">
                                    {orderDetail && orderDetail.is_payment ? "Dish Đã Thanh Toán" : "Dish Đã Phục Vụ"}
                                </h5>
                                <div className="p-0">
                                    {(() => {
                                                ...
                                                {sessions.length > 0 ? (
                                                    sessions.map((session, sIdx) => (
                                                        <div key={sIdx} className="mb-4">
                                                            <div className="bg-secondary p-2 border-start border-4 border-success d-flex justify-content-between align-items-center">
                                                            ...
                                                            <Table variant="dark" striped hover size="sm" className="text-end align-middle mb-0 table-borderless">
                                                                <tbody>
                                                                    {session.items.map((item, index) => (
                                                                        <tr key={index}>
                                                                            <td className="text-start ps-3 fw-bold" style={{ width: '60%' }}>{item.product_name}</td>
                                                                            <td className="text-center fw-bold text-primary" style={{ width: '10%' }}>x{item.qty}</td>
                                                                            <td className="fw-bold text-end pe-3" style={{ width: '30%' }}>{(item.price * item.qty).toLocaleString()}VND</td>
                                                                        </tr>
                                                                    ))}
                                                                </tbody>
                                                            </Table>
                                                    ...
                                                    <div className="text-center p-4 text-muted fst-italic">Chưa có món nào VNDược phục vụ</div>
                                                )}
                                            </>
                                        );
                                    })()}
                    ...
                    {canceledItems.length > 0 && (
                        <div className="order__detail-group mt-4 border rounded bg-dark text-white shadow-sm overflow-hidden canceled-section no-print">
                            <h5 className="p-3 bg-danger text-white mb-0 fst-italic">Dish Đã Cancel</h5>
                            <div className="p-0">
                                <Table variant="dark" striped hover size="sm" className="text-end align-middle mb-0 table-borderless">
                                    {/* <thead className="table-dark">
 <tr style={{ fontSize: '13px' }}>
 <th className="text-start ps-3" style={{ width: '60%' }}>Name món</th>
 ...
 <th className="text-end pe-3" style={{ width: '30%' }}>Nguyên giá</th>
                                    ...
                                    <tbody>
                                        {canceledItems.map((item, index) => (
                                            <tr key={index} className="text-muted">
                                                ...
                                                <td className="text-end pe-3 fw-bold">{(item.price * item.qty).toLocaleString()}VND</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </Table>
                                <div className="p-2 bg-secondary text-end pe-3">
                                    <small className="text-danger fst-italic">* Các món này has been trừ khỏi tổng hóa order</small>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {orderDetail && orderDetail.split_bills && orderDetail.split_bills.length > 0 && (
                        <div className="order__detail-group mt-4 p-3 border rounded border-warning">
                            ...
                            <Table variant="dark" bordered size="sm" className="text-center align-middle">
                                <thead className="table-dark">
                                    <tr>
                                ...
                                <tbody>
                                    {orderDetail.split_bills.map((sb, sbIdx) => (
                                        <tr key={sbIdx} className={sb.is_payment ? "table-dark" : ""}>
                                            <td className="fw-bold align-middle">
                                                {sb.user_name} <br />
                                                <small className="text-muted">({sb.percent}%)</small>
                                            </td>
                                            <td className="text-danger fw-bold align-middle text-center">{sb.amount.toLocaleString()} VND</td>
                                            ...
                                            <td className="align-middle text-center">
                                                {sb.is_payment ? (
                                                    <span className="badge bg-success px-3 py-2">
                                                        ...
                                                        {sb.payment_method === 'cash' ? 'Đã trả cash' : 'Đã transfer'}
                                                    </span>
                                                ) : (
                                                    <div className="py-2 text-center">
                                                        <span className="badge bg-warning px-3 py-2">Chưa thanh toán</span>
                                                    </div>
                                                )}
                                            </td>
                                                ...
                                                <div className="d-flex justify-content-center align-items-center gap-2">
                                                    {!sb.is_payment ? (
                                                        ...
                                                        <button className="btn btn-sm btn-dark text-white fw-bold shadow-sm px-3" onClick={() => setPrintSplitBill(sb.split_id)}>
                                                            <i className="fa-solid fa-print me-2"></i> In lại hóa order
                                                        </button>
                                                    )}
                                                </div>
                                            </td>
                    ...
                    <div className="order__detail-foot mt-4 pb-3 border-bottom-0">
                        <div className="order__detail-status bg-secondary p-4 rounded w-100 d-flex flex-column align-items-end">
                            <div className="d-flex justify-content-between w-100 mb-2 border-bottom pb-2">
                                <span className="text-muted">Status thanh toán:</span>
                                <span className={`badge ${orderDetail && orderDetail.is_payment ? 'bg-success' : (orderDetail && orderDetail.split_bills?.some(sb => sb.is_payment) ? 'bg-warning ' : 'bg-danger')} fs-6`}>
                                    {orderDetail && orderDetail.is_payment ? 'Đã thanh toán VNDủ' : (orderDetail && orderDetail.split_bills?.some(sb => sb.is_payment) ? 'Payment 1 phần' : 'Chưa thu VNDủ')}
                                </span>
                            </div>
                            {orderDetail && orderDetail.is_payment && (!orderDetail.split_bills || orderDetail.split_bills.length === 0) && (
                                <div className="d-flex justify-content-between w-100 mb-2 border-bottom pb-2">
                                    ...
                                    <span className="fw-bold text-primary">
                                        {orderDetail.payment_method && (orderDetail.payment_method.toLowerCase().includes('transfer') || orderDetail.payment_method.toLowerCase().includes('payos')) ? 'Bank Transfer' : 'Cash'}
                                    </span>
                                </div>
                            )}
                            <div className="d-flex justify-content-between w-100 mt-2 align-items-center">
                                <span className="fw-bold fs-5">
                                    {orderDetail && orderDetail.is_payment ? 'Đã Thu (Total):' : 'Total Cần Thu (Số dư nợ):'}
                                </span>
                    ...
                    <div className="order__detail-group-btn mt-3 no-print d-flex justify-content-end bg-secondary p-3 rounded">
