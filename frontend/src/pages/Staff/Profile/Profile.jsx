...
function Profile() {
 ...
 if (response.ok) {
 toast.success("Update ảnh representative/avatar thành công");
 // Update lại state và sessionStorage
 const updatedUser = { ...userUpdate, avatar: data.avatar };
 setUserUpdate(updatedUser);
 sessionStorage.setItem("user", JSON.stringify(updatedUser));
 } else {
 toast.error(data.message ||"Error cập nhật ảnh representative/avatar");
 // Rollback nếu lỗi
 setSelectedImage(null);
 }

 } catch (error) {
 toast.error("Error kết nối khi cập nhật ảnh");
 setSelectedImage(null);
 }
 }
 };
 ...
 // Form submit thông thường không cần gửi avatar nữa vì VNDã tự update khi click
 ...
 toast.error(data.message ||"Error cập nhật thông tin");
 }

 } catch (error) {
 toast.error("Error cập nhật thông tin");
 }
 };
